...
! Local:
   ...
   if (.not. gridstruct%bounded_domain) &
      call copy_corners(qmt, npx, npy, 2, gridstruct%bounded_domain, bd, &
                         gridstruct%sw_corner, gridstruct%se_corner, gridstruct%nw_corner, gridstruct%ne_corner)

   call yppm(fy2, qmt, cry, ord_in, isd,ied,isd,ied, js,je,jsd,jed, npx,npy, gridstruct%dya, &
   ...
   if (.not. gridstruct%bounded_domain) &
     call copy_corners(qmt, npx, npy, 1, gridstruct%bounded_domain, bd, &
                       gridstruct%sw_corner, gridstruct%se_corner, gridstruct%nw_corner, gridstruct%ne_corner)

   call xppm(fx2, qmt, crx, ord_in, is,ie,isd,ied, jsd,jed,jsd,jed, npx,npy, gridstruct%dxa, &
             gridstruct%bounded_domain, gridstruct%grid_type, lim_fac)
